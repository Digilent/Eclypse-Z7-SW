 ...
 * @param buf_num the number of buffers inside the DMA,
 * 	if greater than 1, the DMA will start a cyclic transfer
 * 	otherwise the DMA will start a normal transfer
 * @param transfer_size the size of the transfer in bytes
 *
 * @return 0 on success, any other number on failure
 */
int fnOneWayDMATransfer(uintptr_t addr, uint32_t *buf, int buf_num, size_t transfer_size)
{
	...
	return axidma_oneway_transfer(dma_env->dma_inst, dma_env->channel_id,
			(void *)buf, buf_num, transfer_size, 0);
}
